...
var ProxyServer = function ProxyServer() {
    ...
    var sessionStorage = new SessionStorage(),
        ...
        httpHandlerFunction = buildHandlerFunction(80),
        httpsHandlerFunction = buildHandlerFunction(443),
        httpProxyServer = httpProxy.createServer({
            enable: {
                xforward: false
            }
        }, httpHandlerFunction),
        httpsProxyServer = httpProxy.createServer({
            enable: {
                xforward: false
            },
//            https: httpsOpts
        }, httpsHandlerFunction),
        httpProxyObj = httpProxyServer.proxy,
        httpsProxyObj = httpsProxyServer.proxy,
        listening = false,
    ...
    httpProxyServer.on('upgrade', function (req, socket, head) {
        httpProxyObj.proxyWebSocketRequest(req, socket, head);
    });
    httpsProxyServer.on('upgrade', function (req, socket, head) {
        httpsProxyObj.proxyWebSocketRequest(req, socket, head);
    });
    ...
    this.start = function (done) {
        ...
        var startSession = this.startSession.bind(this);
        var endSession = this.endSession.bind(this);
        [httpProxyObj, httpsProxyObj].forEach(function (httpOrHttpsProxy) {
            httpOrHttpsProxy.on('start', startSession)
        });
        [httpProxyObj, httpsProxyObj].forEach(function (httpOrHttpsProxy) {
            httpOrHttpsProxy.on('end', endSession)
        });
        httpProxyServer.listen(8888, function () {
            httpsProxyServer.listen(8887, function () {
                listening = true;
                done();
            });
        });
    ...
    this.stop = function (done) {
        ...
        httpProxyObj.removeAllListeners('start');
        httpsProxyObj.removeAllListeners('start');
        httpProxyObj.removeAllListeners('end');
        httpsProxyObj.removeAllListeners('end');
        if (listening) {
            httpProxyServer.close(function () {
                httpsProxyServer.close(function () {
                    listening = false;
                    done();
                });
            });
